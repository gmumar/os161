...
static
void
kill_curthread(u_int32_t epc, unsigned code, u_int32_t vaddr, struct trapframe *tf)
{
	(void)tf;
...
mips_trap(struct trapframe *tf)
{
	...
	if (!iskern) {
		 ...
		 */
		DEBUG(1,"fatal fault is user mode %d\n",code);
		kill_curthread(tf->tf_epc, code, tf->tf_vaddr, tf);
		goto done;
	}
