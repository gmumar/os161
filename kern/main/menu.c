...
common_prog(int nargs, char **args)
{
	...
	for(i=0;i<nargs;i++){
		mem_args[i] = (char *)kmalloc(sizeof(char)*(strlen(args[i])+1));
	}
	...
	result = thread_fork(args[0] /* thread name */,
			mem_args/* thread arg */, nargs /* thread arg */,
			cmd_progthread, &ret);
	...
	while((ret->running)==1);//menu waiting here for thread to exit
	i=5;
	return 0;
}
//add 
/*********************************************************************************************************************
dbflags functionality, for asst0
*/
...
//shows the dbflags menu
static int cmd_dbflags(){//int nargs, char **args

	showmenu("OS/161 Debug flags\n", dbflagsmenu);//Causes a warning -> `showmenu' was declared implicitly `extern' and later `static' 	
	kprintf("Current value of dbflags is 0x%x\n", dbflags);
	return 0;
}

//parses the df input to change the dbflags value from lib.h
static int df_parse(int nargs, char **args){
